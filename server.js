...
app.get("/shop/:id", (req, res) => {
  ...
  Promise.all([
    ...
    itemData.getPublishedItemsByCategory(category),
    itemData.getAllCategories(),
  ])
...
app.get("/shop", (req, res) => {
  ...
  Promise.all([getItems, getCategories])
    .then(([items, categories]) => {
      let post, posts;
      if (items.length > 0) {
        post = items[0]; // first item will be the main post
        posts = items.slice(1); // rest of the items will be in the sidebar
      }
